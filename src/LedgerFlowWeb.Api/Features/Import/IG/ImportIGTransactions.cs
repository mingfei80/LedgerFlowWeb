...
public static class ImportIGTransactions
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var account = await dbContext.Accounts
            .FirstOrDefaultAsync(
                a => a.Id == accountId && a.UserId == appUser.Id,
                cancellationToken);
        ...
        {
            return Results.BadRequest(new { message = "Selected account is not an IG account." });
        }
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            dbContext.PlatformImports.Add(new PlatformImport
            {
            ...
            var mapped = MapRow(row, appUser.Id, account.Id);
            if (!mapped.Success || mapped.Transaction is null)
            {
                ...
                dbContext.SkippedTransactions.Add(new SkippedTransaction
                {
                    ImportLogId = importLog.Id,
                    UserId = appUser.Id,
                    AccountId = account.Id,
            ...
            mapped.Transaction.SecurityId = await ResolveSecurityIdAsync(dbContext, mapped, cancellationToken);

            var duplicate = await dbContext.Transactions
                .FirstOrDefaultAsync(
            ...
            dbContext.Transactions.Add(mapped.Transaction);
            await dbContext.SaveChangesAsync(cancellationToken);
            ...
            dbContext.TransactionDetails.Add(new TransactionDetail
            {
                ...
                ParsedExtrasJson = JsonSerializer.Serialize(new
                {
                    ...
                    row.DateUtc,
                    row.OpenDateUtc,
                    ExtractedExchangeRate = mapped.Transaction.ExchangeRate
                }),
                CreatedAt = DateTime.UtcNow
            });
        ...
        importLog.Status = importLog.ErrorCount == 0
            ? ImportStatus.Success
            : importLog.ImportedCount > 0
                ? ImportStatus.PartialSuccess
                : ImportStatus.Failed;

        importLog.DurationMs = (long)(DateTime.UtcNow - startedAt).TotalMilliseconds;
        importLog.ErrorDetails = errors.Count == 0 ? null : JsonSerializer.Serialize(errors);
        ...
        return Results.Ok(new
        {
            ...
            importLog.SkippedCount,
            importLog.ErrorCount,
            importLog.DurationMs
        });
    }

    private static async Task<List<IGCsvRow>> ReadRowsAsync(IFormFile file, CancellationToken cancellationToken)
    {
    ...
    private static TransactionType MapTransactionType(IGCsvRow row)
    {
        ...
        if (summary.Equals("Client Consideration", StringComparison.OrdinalIgnoreCase))
        {
            if (code == "WITH")
            {
    ...
    private static (decimal quantity, decimal unitPrice) ParseQuantityAndPrice(string? marketName)
    {
        ...
        var match = Regex.Match(
            marketName,
            @"\b(?:CONS|DIVIDEND)\s+([0-9]+(?:\.[0-9]+)?)@([0-9]+(?:\.[0-9]+)?)",
